...
I've written previously on [Python Packaging](https://ns-rse.github.io/posts/python-packaging) and in that article included details of how to [automate publishing to PyPI
from GitHub](https://ns-rse.github.io/posts/python-packaging/#publishing-to-pypi). This article details how to automatically publish your package to [PyPI](https://pypi.org) from [GitLab](https://gitlab.com).

![[Packages with Legs, by Me](https://flickr.com/photos/slackline/52985552723/in/dateposted/)](https://live.staticflickr.com/65535/52985552723_a975753e12_k.jpg)
...
publishing on PyPI or Test PyPI. These are saved under the repository `_Settings > CI/CD > Varialbes_` section and how to
create and save these is described below in the [PyPI Tokens](#pypi-tokens)

### Protecting Tags

This really stumped me I could build and push automatically from the `master` branch but could not use the `- if
$CI_COMMIT_TAG` condition to publish commits that were tagged. I wrote a [post on the GitLab Forums](https://forum.gitlab.com/t/ci-variables-missing-when-triggering-build-based-on-tag-solved/93309) asking how to do this
and posted it to Mastodon asking if anyone had any ideas. I got two replies (one from [@manu_faktur@mastodon.social](https://mastodon.social/@manu_faktur/111159226739296263) and
one from [@diazona@techhub.social](https://mastodon.social/@diazona@techhub.social/111159429646899283)) both asking if I'd protected the tags on my repository.

I had no idea that you could protect tags on GitLab (or GitHub for that matter) so looked up the documentation on
[Protected tags](https://docs.gitlab.com/ee/user/project/protected_tags.html) and sure enough this was possible. After setting a [wildcard to protect my tags](https://docs.gitlab.com/ee/user/project/protected_tags.html#wildcard-protected-tags) the `pypi` CI job
defined [below](#ci-configuration) worked as expected and built on tagged commits.
...
An example file from the [tcx2gpx](https://gitlab.com/nshephard/tcx2gpx/) package is shown below (see [here](-/blob/master/.gitlab-ci.yml?ref_type=heads)).

This defines the following...

+ `image` - the use of a Docker Python 3.11 image for running the pipeline.
+ `variables` - Configures [pre-commit](https://ns-rse.github.io/#category=pre-commit) to run and automatically fix issues found on pull requests.
...
It GitLab got to your repositories _Settings > CI/CD > Variables_ and add two new variables `TWINE_USERNAME` with the value
`__token__` and `TWINE_PASSWORD` with the token for your account on Test PyPI (remember to prefix it with `pypi-` as shown in
the above example `.pypirc`). You have options on how these variables are used and should ensure that all three check
boxes are selected, this enables...
