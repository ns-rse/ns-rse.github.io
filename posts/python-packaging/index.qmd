...
The `dependencies` are those that are required for running the code. They should not include packages that are required
for development (e.g. `black`. `flake8`, `ruff`, `pre-comit`, `pylint` etc.), nor those required for testing
...
sets the package version based on the version control data. It is important to note that by default `setuptools_scm`
