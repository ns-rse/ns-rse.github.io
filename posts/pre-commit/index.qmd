...
There is also [pre-commit.ci](https://pre-commit.ci/) which offers integration with GitHub (GitLab and other forges are
in the pipeline). This "_enforces that issues are descovered but also fixes the issues automatically, letting developers
focus their time on more valuable problems_". Configuration is via the `.pre-commit-config.yaml` under a dedicated `ci:`
section and a default configuration is shown below.
...
Its possible to explicitly skip particular tests within this pipeline via a list under `skip: []`.
