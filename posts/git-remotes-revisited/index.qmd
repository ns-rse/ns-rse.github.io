...
careful in how you use this setup. The original StackOverflow post that showed me how to do this suggests, you would
probably want to leave `origin`  as is and setup another `remote` (e.g. `all`) so that you can push changes to `origin`
and `all` (and any other remotes you setup) independently.
