...
[Pytest][pytest] has a few [decorators][rp_decorators] that available to help with tests, you
...
The `pytest.mark.skipif()` and `pytest.mark.xfail()` functions argument `condition` is still valid when used here so we
can conditionally mark parameterised tests to fail. The `cond` should evaluate to `True` or `False` and a `reason`
argument still needs to be given.
...
This post was stems from a suggestion made by [@jni@jni@fosstodon.org][jni] during some work I have been contributing to
the [skan][skan] package. Thanks to Juan for the prompt/pointer.
