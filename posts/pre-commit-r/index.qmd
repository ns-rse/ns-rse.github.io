...
[precommit](https://cran.r-project.org/web//packages/precommit/vignettes/precommit.html) Package. This shouldn't be
confused with [pre-commit](https://ns-rse.github.io/pre-commit) which is a Python Package. The R package (`precommit`)
consists of a number of R specific hooks that are run by `pre-commit` before commits are made and check various aspects
of your code for compliance with certain style and coding standards (mostly aspects of R packages and I'll be posting
more on R packaging in due course).

A major part of this post is about getting things setup on Windows, I've only given a light overview of some of the hooks
and common problems encountered as I've gone about using and learning R packaging. As not everyone uses Windows that is
addressed in the second half of the article.

Most of the work on this package is by [Lorenz Walthert](https://github.com/lorenzwalthert/), if you find it useful
...
hassle (once you are used to it). The [R precommit
package](https://github.com/lorenzwalthert/precommit) comes with a set of hooks that can be enabled and configured
individually, but as I've not used R much for a few years and this was my first time attempting to develop a package,
it took me a little figuring out to get them working.

I opted to enable _all_ of the hooks. I've not covered them all here (yet) but describe some of them below.
...
output the YAML that you need to add to your `.pre-commit-config.yaml`. It might look something like the following.
...
that I started developing so I needed to get all members of the team up and running with the `precommit` R
package/pipeline.
...
provided in the [precommit vignette](https://cran.r-project.org/web/packages/precommit/vignettes/precommit.html) and
uses the [reticulate](https://rstudio.github.io/reticulate/) package (which interfaces [R](https://r-project.org) with
...
The [usethis](https://usethis.r-lib.org) package is a compliment to the [devtools](https://devtools.r-lib.org) package that has a lot of very useful helper functions.
