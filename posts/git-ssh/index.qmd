...
Typically when pushing changes to a forge such as [GitHub](https://github.com), [GitLab](https://gitlab.com) or
[Codeberg](https://codeberg.org) you use an SSH (*S*ecure *SH*ell) key to authenticate that you have permission to.
...
Under your account settings on your chosen Git Forge navigate to _Settings > SSH and GPG Keys_ and select _Add New Key_
on ([GitHub](https://github.com/settings/keys)). On GitLab navigate to _Preferences > SSH Keys_
...
`identity_file`) that is located at `~/.ssh/different_ed25519`. Providing you have...

a) Already uploaded the public key (`different_ed25519.pub`) to your GitHub account.
...
`origin` (typically a forge such as GitHub, GitLab, Codeberg or so forth).


## Commit verification with SSH

Verification of commits is a usefl security feature, but beyond the scope of this article but as doing so with SSH keys is a
recently supported feature on GitHub (see blog [SSH commit verification now
supported](https://github.blog/changelog/2022-08-23-ssh-commit-verification-now-supported/)) I felt it worth
mentioning.
