...
ensure that pull requests that are made against your repository continue to pass all tests. Here we add it under the
`dev` subset of optional dependencies in the projects `pyproject.toml` as only those developing the software will want
to install it, and we want to avoid installing it unnecessarily in say Continuous Integration.
...
typically the whole test suite will be run in CI anywaym and give it appropriately named `ìd` and `name` values. The
