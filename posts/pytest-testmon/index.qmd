...
tests. For example if you want to only run an individual test within `tests/test_submodules.py` you can do that too just
specify the name of the test you want to run after the test file name, so if you had `test_function1()` defined in
`tests/test_submodules.py` you would run the following.
...
As you iterate through the development cycle you could manually run your tests which works fine, but that doesn't
account for any integration tests where your functions are used as part of a pipeline and having to know what tests are
affected by changes can be challenging.
...
run. But it's still running the tests manually before you make your `git commit`. We want to automate this step, can we
do it so that `pytest --testmon` is run prior to commits on only the changed files?
...
appropriately named `ìd` and `name` values. The `entry` is the command we wish to run, i.e. `pytest --testmon` and we
