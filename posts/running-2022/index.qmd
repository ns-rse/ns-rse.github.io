...
table](https://orgmode.org/manual/Tables.html). For the purpose of this post I have saved this to a `.RData` file having
done some redaction and data formatting so that it can be included in the page itself. The following code chunk
determines where to load the file from. If I'm working on the file locally the original data is loaded and parsed before
saving to `.RData`. In GitHub pages, the Action (CI/CD) pipeline won't have access to the original instead it load the
`.RData` file and its good to go with generating graphs and renders correctly.
...
For some reason this value isn't correctly shown in the table below.
...
(post-stretching!) I don't always do so and so the times below reflect the times I logged the climb rather than
started.
...
(min+s)/km);L`) calculates this automatically on saving.
...
`~/path/to/running.log`.  This file forms part of my `training.org` (its pulled into the main document `training.org`
using `#+INCLUDE: ~/path/to/running.org`), but the data is used and processed using [R](https://www.r-project.org). How
does the data get from the org-format table into the R session as a data frame for summarising and using? This is part
of the amazing magic that is [Org-babel](https://orgmode.org/worg/org-contrib/babel/) for literate programming. A source
code chunk can be defined with a `:var` option which refers to the table you want to include. In this my source block
does some processing of the table to tidy up the dates into something R understands and is shown below.
