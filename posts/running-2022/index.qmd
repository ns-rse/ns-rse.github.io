...
I started running :running: in 2019. It never appealed but I needed to get regular exercise easily from my doorstep as
despite commuting by bike for most of my life and cycling between 12-20km daily it wasn't enough for me. Whilst I've
always cycled for commuting and cycling was delightful during the pandemic lockdowns I've since enjoyed it an increasing
number of motorists seen to have little to no regard for the safety of pedestrians and cyclists. As a consequence I
prefer running over cycling these days and get most of my aerobic exercise on two feet rather than two wheels.
...
knowledge, the code chunks are hidden by default but can be easily expanded if you wish to look at them. I've summarised
some of the features I've learnt about Quarto and I've explained how I use [Emacs Org-mode to capture my
...
table](https://orgmode.org/manual/Tables.html). For the purpose of this post I have saved this to a CSV having done some
redaction and data formatting so that it can be included in the page itself. The following code chunk determines where
to load the file from. If I'm working on the file locally the original data is loaded and parsed before
writing to CSV. In GitHub pages, the Action (CI/CD) pipeline won't have access to the original and so by saving it as a
CSV file and including it as part of the repository it means I can instead load the file the code that follows
generating graphs and renders correctly.
...
**TODO** - Calculate the interquartile range, tricky with `across()` used above? :thinking:
...
hold a conversation then I'm not trying hard enough (in reality I rarely try as I always run on my own).
...
I think this is what causes the dip in the below graph (excluding outliers) in the range of 11-14km, but further
...
#| fig-alt: "Time of day I go running"
...
I use [OpenTracks](https://opentracksapp.com/) to log my runs. This saves data to a [GPX]() file on finishing on my
phone and I use [SyncThing](https://syncthing.net) to back these up automatically to my server using a run condition so
it only tries to do so when the phone is connected to my home WiFi network and the phone is plugged in. At some point I
will take advantage of this and develop a dashboard that loads the GPX data and allows interactive exploration as well
as summarising the data similar to that which I've presented here. But life is busy so for now I manually capture the
summary statistics using [Org-mode](https://orgmode.org) and [Org Capture
templates](https://orgmode.org/manual/Capture.html).

I have a file which contains and [Org table](https://orgmode.org/manual/Tables.html) as shown below.
...
~/path/to/running.org`, navigate to the top of the file and start entering details manually. But that is a bit slow and
instead I wrote an [Org-capture](https://orgmode.org/manual/Capture.html) rule to capture runs (and many other
things). Writing these was initially a bit tricky (as I'm a slow learner) but I now understand the structure and can
quickly add new entries to capture items where I want them to be, saving me time in the long run.
...
the field content is `%?` which means user input is required. The `Distance`, `Time` and `Notes` fields are completed
although the Pace field should be left blank since the formula at the bottom of the table (`#+TBLFM: $5=uconvert($4/$3,
(min+s)/km);L`) calculates this automatically for on on saving

``` lisp
Capture buffer, Finish 'C-c C-c', refile 'C-c C-w', abort 'C-c C-k'
| [2022-12-27 Tue 11:29] |  | km | min + s | | |
```

Once all the fields are completed press `C-c C-c` to save the changes and the row is added to the table in the file
`~/path/to/running.log`.  This file forms part of my `training.org` (its pulled in using the main document `training.org`
using `#+INCLUDE: ~/path/to/running.org`), but the data is used and processed using [R](https://www.r-project.org). How
does the data get from the org-format into the R session for summarising and using? This is part of the amazing magic
that is Org-mode and Org-babel for literate programming. A source code chunk can be defined with a `:var` option which
refers to the table you want to include. In this my source block does some processing of the table to tidy up the dates
into something R understands and is shown below.
...
table available in the R session as the dataframe `running_table`.


# Quarto - Things I've learnt

Here are a list about Quarto of things I've learnt whilst preparing this document
...
I'm using the [blogging](https://quarto.org/docs/websites/website-blog.html) feature of Quarto and
so tried as well as adding it to the YAML header for the post itself (i.e. `posts/running-2022/index.qmd`) but it didn't
seem to have any effect, the code chunks were still displayed. I could however set this on a per-chunk basis though so
each code chunk carries the options.
...
## Table and Figure caption locations are configurable

You can specify the location of [table](https://quarto.org/docs/authoring/tables.html#caption-location) and
...
Its possible to include two figures in a code chunk and have them both displayed.
...
In the output format of the blog these do _not_ appear side by side, but rather underneath each other. Captions were by
default underneath each picture which is perhaps ok when reading as a PDF but this is rendered as HTML and I would
prefer these to be at the top so that readers see the heading as they scroll down (I'm often torn about figure headings
and labels and feel they should be included in the image itself so they are retained if/when they are used elsewhere).
...
# R - Things I've ~~remembered~~ forgotten

Its been a few years since I used R on a daily basis when working as a medical statistician. These days I mostly use
Python and do very little data analysis. As a consequence I thought I'd forgotten a bunch of things I used to know how
to do, but this exercise has reminded me that I perhaps have some vestigial remnants of my old knowledge
lingering. Some things I had to look up and, unsurprisingly, the available tools, functions/methods have evolved in that
time (viz. [tidyr](https://tidyr.tidyverse.org/) is now more generic and simpler than [reshape2]()).
...
rather than leaving them as POSIXct objects and then trying to format them in `ggplot2`.
...
Whilst I'm familiar with virtual environments under Python this is something relatively new to me for R so I won't write
